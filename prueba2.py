...
while running:
    ...
    # ... código para manejar el movimiento de las piezas ...
            ...
            if event.type == pygame.MOUSEBUTTONDOWN: # Obtener la posición del clic 
                ...
                if movimiento in movimientos_seleccionados:# Verificar si el movimiento es válido y que su rey no queda en jaque
                    ...
                    if not tablero_temporal.is_attacked_by(piezas_atacantes, rey_posicion): # verificar que el rey del que realiza el movimiento no quede en jaque            
                        tablero.push(movimiento) #revertir el movimiento, porque deja al rey en jaque
                        turno = not turno
                    
                    # Detectar si un peón alcanza la última fila
                    if movimiento.piece_type == chess.PAWN and (movimiento.to_square in chess.SquareSet(chess.RANK_7) or movimiento.to_square in chess.SquareSet(chess.RANK_0)):
                        nueva_pieza = chess.QUEEN
                        tablero.set_piece_at(movimiento.to_square, chess.Piece(nueva_pieza, tablero.turn)) # Actualizar el tablero reemplazando el peón por la nueva pieza
